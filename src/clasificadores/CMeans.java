...
public class CMeans {
    ...
    private final ArrayList<Patron> instancias;
	private final ArrayList<Patron> instancias2;
    // numero de clusters
    private final int c;
    // centroides 
    private final ArrayList<Patron[]> centroides;
    int[] contadores;  
    public CMeans(ArrayList<Patron> instancias, int c) {
        this.instancias = instancias;
        this.centroides = new ArrayList<>();
        this.c = c;
		this.instancias2 = instancias;
    }
...
//cambiar los vectores del pixel por el de los centroides
    private void cambioVectores() {
        Patron[] c = centroides.get(centroides.size()-1);
        for(int i=0;i<instancias2.size();i++){
			for (Patron c1 : c) {
				if (instancias2.get(i).getClase().equals(c1.getClase())) {
					instancias2.get(i).setVector(c1.getVector());
					break;
				}
			}
        }
